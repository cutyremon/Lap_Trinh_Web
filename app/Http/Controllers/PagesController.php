...
class PagesController extends Controller
{
    ...
    {
        $user = Auth::user();
        $products = DB::table('products')->join('cart', 'products.id', '=', 'cart.product_id')->where('cart.user_id','=',Auth::user()->id)
        ->distinct()->get();
    ...
    { 
        ...
        // dd($a[0],$a[1]);
        $i = 0;
        $carts = Cart::orderBy('created_at', 'asc')->where('cart.user_id','=',Auth::user()->id)->get();
